...
async def _llm_inference_task(
    ...
    tools = tool_ctx.all_tools  # 1.3.10: includes both function_tools and provider_tools
...
class _TTSGenerationData:
    ...
    collected_timed_words: list[io.TimedString] = field(default_factory=list)
            ...
            if timed_text_ch is not None:
                for text in audio_frame.userdata.get(USERDATA_TIMED_TRANSCRIPT, []):
                    if isinstance(text, io.TimedString):
                        ...
                        data.collected_timed_words.append(text)
    ...
    try:
        async for fnc_call in function_stream:
            ...
            try:
                ...
                # Record per-tool duration when each tool finishes
